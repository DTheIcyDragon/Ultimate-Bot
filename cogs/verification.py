...
class Verification(commands.Cog):
    ...
    async def verify(self, ctx: ApplicationContext):
    

        view = utils.buttons.VerificationButtons()
        msg = await ctx.respond(embeds = [introduction, rule1, rule2, rule3, rule4, disclaimer], view = view)
        view.message = msg
    ...
    async def on_member_join(self, member):
        ...
        verify_category = self.client.get_channel(int(os.getenv("VERIFY")))
        channel = await verify_category.create_text_channel(
        ...
        msg = await channel.send(
            ...
            embeds = [introduction, rule1, rule2, rule3, rule4, disclaimer],
            view = view)
        view.message = msg
